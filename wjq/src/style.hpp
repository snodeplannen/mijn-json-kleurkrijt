...
struct StyleRule {
  ...
  std::optional<std::string> array_index_pattern; // "even", "odd", "first", "last", "every:N"
  ...
  bool matches(ElementType type, const StyleContext &ctx) const {
    ...
    if (!element_types.empty()) {
      bool type_matches =
          std::find(element_types.begin(), element_types.end(), type) != element_types.end();
      if (!type_matches)
        return false;
    }
  ...
  std::unique_ptr<StyleRule> clone_unique() const {
    return clone();
  }
};
...
struct Style {
  ...
  Color get_base_color(ElementType type) const {
    switch (type) {
    case ElementType::Key: return key_color;
    case ElementType::String: return string_color;
    case ElementType::Number: return number_color;
    case ElementType::Boolean: return bool_color;
    case ElementType::Null: return null_color;
    case ElementType::Brace: return brace_color;
    case ElementType::Bracket: return bracket_color;
    case ElementType::Colon: return colon_color;
    case ElementType::Comma: return comma_color;
    }
