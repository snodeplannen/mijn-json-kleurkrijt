...
QueryValue JsonPathEngine::execute(const JsonPathNode* path, const QueryValue& root) {
    if (!path) return QueryValue::null();
...
QueryValue JsonPathEngine::execute(const JsonPathNode* path, simdjson::ondemand::document& doc) {
    auto root = QueryValue::fromSimdjson(doc);
    return execute(path, root);
}

std::vector<QueryValue> JsonPathEngine::evaluate_node(const JsonPathNode* node, const QueryValue& context) {
    ...
    switch (node->type) {
            ...
            if (context.isObject()) {
                ...
                for (const auto& [key, value] : obj) {
                    if (key == node->property_name) {
                        // std::cerr << "DEBUG Property " << node->property_name << " found, hasNext=" << (node->next ? 1 : 0) << "\n";
                        if (node->next) {
        ...
        case JsonPathNodeType::Slice: {
            if (context.isArray()) {
                ...
                if (start < 0) start = static_cast<int>(arr.size()) + start;
                if (end < 0) end = static_cast<int>(arr.size()) + end;
                if (end < 0 || end > static_cast<int>(arr.size())) end = static_cast<int>(arr.size());
        ...
        case JsonPathNodeType::RecursiveDescent: {
            // std::cerr << "DEBUG RecursiveDescent, next type=" << (node->next ? (int)node->next->type : -1) 
            //           << " next->next type=" << ((node->next && node->next->next) ? (int)node->next->next->type : -1) << "\n";
...
std::vector<QueryValue> JsonPathEngine::evaluate_recursive(const JsonPathNode* next, const QueryValue& context) {
    std::vector<QueryValue> results;
...
std::vector<QueryValue> JsonPathEngine::evaluate_recursive_with_filter(
    const JsonPathNode* target, 
    const QueryValue& context, 
    const JsonPathNode* filter_node,
    ...
    if (context.isObject()) {
        const auto& obj = context.asObject();
        for (const auto& [key, value] : obj) {
            auto sub = evaluate_recursive_with_filter(target, value, filter_node, root);
            results.insert(results.end(), sub.begin(), sub.end());
        }
    } else if (context.isArray()) {
        const auto& arr = context.asArray();
        for (const auto& item : arr) {
            auto sub = evaluate_recursive_with_filter(target, item, filter_node, root);
            results.insert(results.end(), sub.begin(), sub.end());
        }
    }
...
bool JsonPathEngine::evaluate_filter(const FilterExpr* expr, const QueryValue& context, const QueryValue& root) {
    if (!expr) return true;
            ...
            switch (expr->op) {
                ...
                case FilterOp::Lt: {
                    if (left_val.isNumber() && right_val.isNumber()) {
                        ...
                        std::cerr << "DEBUG: " << left_val.asNumber() << " < " << right_val.asNumber() << " = " << result << "\n";
                        return result;
                    }
...
QueryValue JsonPathEngine::evaluate_filter_value(const FilterExpr* expr, const QueryValue& context, const QueryValue& root) {
    if (!expr) return QueryValue::null();
        ...
        case FilterExpr::Type::Path: {
            ...
            while (pos < path_str.size() && current) {
                if (path_str[pos] == '.') {
                    pos++;
                    std::string prop;
                    while (pos < path_str.size() && (std::isalnum(path_str[pos]) || path_str[pos] == '_')) {
                        prop += path_str[pos++];
                    }
                    ...
                    if (current->isObject()) {
                        ...
                        if (!found) return QueryValue::null();
                    } else {
                        return QueryValue::null();
                    }
...
std::string JsonQuery::execute(const std::string& json, const std::string& query) {
    last_error_.clear();
    ...
    try {
        ...
        if (is_jsonpath(query)) {
            ...
            // Use simple query engine for jq-style
            std::string jq_result = SimpleQueryEngine::execute(json, query);
            if (!SimpleQueryEngine::lastError().empty()) {
                last_error_ = "Query error: " + SimpleQueryEngine::lastError();
                return "null";
            }
            return jq_result;
        }
...
std::string JsonQuery::get_error() {
    return last_error_;
}
