...
// Supports basic jq-style filters like: .key, .key.nested, .array[], .array[0], etc.
...
private:
    ...
    static std::string applyQuery(const std::string& json, const std::string& query);
    
    // Query operations
    static std::string getProperty(const std::string& json, const std::string& prop);
...
inline std::string SimpleQueryEngine::lastError() {
    return error_;
}

inline std::string SimpleQueryEngine::execute(const std::string& json, const std::string& query) {
    error_.clear();
...
inline std::string SimpleQueryEngine::applyQuery(const std::string& json, const std::string& query) {
    ...
    while (pos < query.size()) {
        ...
        if (query[pos] == '.') {
            ...
            if (pos < query.size() && query[pos] == '[') {
                ...
                // Array index
                std::string idx;
                while (pos < query.size() && (std::isdigit(query[pos]) || query[pos] == '-')) {
                    idx += query[pos++];
                }
                if (pos < query.size() && query[pos] == ']') {
                    pos++;
...
inline std::string SimpleQueryEngine::getProperty(const std::string& json, const std::string& prop) {
    ...
    if (json[pos] == '{') {
        ...
        while (pos < json.size() && !std::isspace(json[pos]) && json[pos] != ',' && json[pos] != '}' && json[pos] != ']') {
            pos++;
        }
        return json.substr(start, pos - start);
    }
...
inline std::string SimpleQueryEngine::getArrayElement(const std::string& json, int index) {
