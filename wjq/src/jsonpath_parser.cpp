...
std::unique_ptr<JsonPathNode> JsonPathParser::parse_path(std::unique_ptr<JsonPathNode> current) {
    skip_ws();
...
std::unique_ptr<JsonPathNode> JsonPathParser::parse_dot(std::unique_ptr<JsonPathNode> current) {
    skip_ws();
...
std::unique_ptr<JsonPathNode> JsonPathParser::parse_bracket(std::unique_ptr<JsonPathNode> current) {
    if (!consume('[')) {
        error_ = "Expected '['";
        return nullptr;
    }
    ...
    if (peek() == '?') {
        ...
        skip_ws();
        if (!consume(')')) {
            error_ = "Expected ')' at end of filter expression, found: " + std::string(1, peek());
            return nullptr;
        }
        if (!consume(']')) {
...
std::unique_ptr<JsonPathNode> JsonPathParser::parse_recursive(std::unique_ptr<JsonPathNode> current) {
    auto node = std::make_unique<JsonPathNode>(JsonPathNodeType::RecursiveDescent);
    ...
    if (node->next) {
        ...
        if (rest) {
            ...
            while (end->next) end = end->next.get();
