...
### Added
- **wjq compact() function**: Added `compact()` filter to remove keys with empty strings or null values recursively.
- **wjq --clean option**: Added `--clean` CLI flag for easy cleaning of JSON without writing a filter.
- **wjq version bump**: Updated wjq version to 2.1.0.
...
## [0.8.7] - 2026-02-28
### Added
- Interactive documentation gallery for all 22 color themes in README.md.
- Automated Python script for generating library-accurate theme previews.

## [0.8.6] - 2026-02-28

### Fixed
- **Linux Build**: Added `<cmath>` header to `query_executor.cpp` for `fmod`.
- **Compiler Warnings**: Fixed `warn_unused_result` for `simdjson` in `query_engine.cpp`.
