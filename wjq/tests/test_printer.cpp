...
TEST_CASE("Printer default theme boolean coloring", "[printer]") {
  Style s = Style::getPreset("default");
  s.color_mode = ColorMode::TrueColor; // Force colors
  Printer p(s);

  std::string json = R"({"t": true, "f": false})";
  std::string result = p.printFromJson(json);

  // Default theme uses 50;255;50 for true and 255;50;50 for false
  REQUIRE(result.find("50;255;50") != std::string::npos); // green true
  REQUIRE(result.find("255;50;50") != std::string::npos); // red false
}
