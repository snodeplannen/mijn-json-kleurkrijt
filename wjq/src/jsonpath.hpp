...
private:
  ...
  parse_path(std::unique_ptr<JsonPathNode> current);
  std::unique_ptr<JsonPathNode>
  parse_bracket(std::unique_ptr<JsonPathNode> current);
  std::unique_ptr<JsonPathNode>
  parse_dot(std::unique_ptr<JsonPathNode> current);
  std::unique_ptr<JsonPathNode>
  parse_recursive(std::unique_ptr<JsonPathNode> current);
...
public:
  ...
  static QueryValue execute(const JsonPathNode *path,
                            simdjson::ondemand::document &doc);

  // String interface for convenience
  static std::string execute_string(const std::string &json,
                                    const std::string &path);

private:
  // Execution methods
  static std::vector<QueryValue> evaluate_node(const JsonPathNode *node,
                                               const QueryValue &context);
  static std::vector<QueryValue> evaluate_recursive(const JsonPathNode *next,
                                                    const QueryValue &context);
  static std::vector<QueryValue> evaluate_recursive_with_filter(
      const JsonPathNode *target, const QueryValue &context,
      const JsonPathNode *filter_node, const QueryValue &root);

  // Filter evaluation
  static bool evaluate_filter(const FilterExpr *expr, const QueryValue &context,
                              const QueryValue &root);
  static QueryValue evaluate_filter_value(const FilterExpr *expr,
                                          const QueryValue &context,
                                          const QueryValue &root);

  // Helper for recursive descent
  static void collect_all(const QueryValue &value,
                          std::vector<QueryValue> &results);
  static void collect_by_name(const QueryValue &value, const std::string &name,
                              std::vector<QueryValue> &results);
};
...
public:
  ...
  static std::string
  execute(const std::string &json, const std::string &query,
